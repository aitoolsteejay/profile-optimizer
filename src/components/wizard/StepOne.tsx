...
type DataSource = "scraper" | "manual_fallback" | "manual";
...
const StepOne = ({ data, onNext }: StepOneProps) => {
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: result, error } = await supabase.functions.invoke("scrape-linkedin", {
    ...
    <div className="card-elevated p-8 md:p-10 glow-accent animate-scale-in max-w-2xl mx-auto">
      ...
      {!urlSubmitted && (
        <form onSubmit={handleUrlSubmit} className="space-y-6">
          ...
          <Button type="submit" variant="hero" className="w-full" disabled={isScraping}>
            {isScraping ? (
              <>
          ...
          >
            Or enter your profile details manually →
          </button>
        </form>
      )}
      ...
      {urlSubmitted && (
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          {/* Show headline/about fields when fallback or empty */}
